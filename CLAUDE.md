...
**Usage:**
- `node dist/cvm.js install 2.0.37` - Install version locally
- `node dist/cvm.js use 2.0.37` - Switch to version
- `node dist/cvm.js list` - List installed versions
- `node dist/cvm.js current` - Show active version
- `node dist/cvm.js list-remote` - Show all 249 available versions
- `node dist/cvm.js claude --version` - Run Claude with active CVM version
- `node dist/cvm.js uninstall 2.0.37` - Remove version
...
3. **Config** (`src/lib/config.ts`)
   - Type-safe configuration management
   - Persistent storage at `~/.cvm/config.json`

4. **PluginLoader** (`src/lib/plugin-loader.ts`)
   - Dynamic plugin loading from `~/.cvm/plugins/`
...
- Compiled to CommonJS (`dist/` directory)
...
- ✅ Tests for version management core functionality
- Coverage: Version installation, switching, listing, and cleanup
- Plugin system tested with benchmark plugin
...
We use **CodeRabbit** for AI-powered code reviews on this project.

### Quick Setup

For detailed setup instructions and best practices, see **[CODERABBIT_WORKFLOW.md](./CODERABBIT_WORKFLOW.md)**.

**Quick start:**
```bash
# Install CLI (optional for maintainers)
curl -fsSL https://cli.coderabbit.ai/install.sh | sh
coderabbit auth login

# Install GitHub App (required - after initial push)
# Visit: https://github.com/apps/coderabbitai
```

### Workflow

1. **GitHub App (Primary)**: Automatically reviews all PRs
2. **CLI (Optional)**: `coderabbit review --plain` before committing locally

See [CODERABBIT_WORKFLOW.md](./CODERABBIT_WORKFLOW.md) for full documentation.
